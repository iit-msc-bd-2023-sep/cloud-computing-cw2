package com.iitmsc.developeriq.application.aop;
...
public class ApplicationAopAspect {
     ...
     * @throws Throwable
     */
    @Around( "execution(* com.iitmsc.developeriq.application.controller.**.*(..))" )
    public Object logAround(ProceedingJoinPoint joinPoint) throws Throwable {
        ...
        try {
            if ( logger.isInfoEnabled() ) {
                logger.info("{}:{}:{}:{}:{}", Constants.METRICS_MGT, Constants.REQUEST_RECEIVED, function, api, mapper.writeValueAsString(argList[ 0 ]));
            }
        }
        ...
        if ( Boolean.TRUE.toString().equals(yamlConfig.getLoggingResponseShouldLogInfo()) && logger.isInfoEnabled() ) {
            ...
            logger.info(Constants.LOG_STRING, Constants.METRICS_MGT, Constants.REQUEST_TERMINATED, function, api,  responseCode, responseTime, response.getBody());
        } else {
            logger.info("sending response with requestId : {}", MDC.get(yamlConfig.getRequestIdKey()));
            if ( logger.isDebugEnabled() ) {
                logger.debug(Constants.LOG_STRING, Constants.METRICS_MGT, Constants.REQUEST_TERMINATED, function, api, responseCode, responseTime, response.getBody());
            }
        }
     ...
     * @param request
     */
    @Before( "( execution(* com.iitmsc.developeriq.domain.**.*.*(..)) || " +
    ...
    public void beforeDomainMethod(JoinPoint pjp, Object request) {
        ...
        try {
            if ( logger.isDebugEnabled() ) {
                logger.debug(Constants.METHOD_START, Constants.METRICS_MGT,className +":"+ method.getName(),  Objects.nonNull(request)? mapper.writeValueAsString(request):"null");
            }
        }
        catch ( JsonProcessingException e ) {
     ...
     * @param response
     */
    @AfterReturning( value = "execution(* com.iitmsc.developeriq.domain.**.*.*(..))",
            returning = "response" )
    public void afterDomainMethod(JoinPoint pjp, Object response) {
        ...
        try {
            if ( logger.isDebugEnabled() ) {
                logger.debug(Constants.METHOD_OUT, Constants.METRICS_MGT, className +":"+ method.getName(),  Objects.nonNull(response)? mapper.writeValueAsString(response):"null");
            }
        }
        catch ( JsonProcessingException e ) {
            logger.error(Constants.ERROR_PARSING_PARAMS_IN_CLASS, pjp.getTarget().getClass().getSimpleName());
